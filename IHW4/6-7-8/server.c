...
void addClient(int socket) {
    ...
    clients[client_counter].id = client_counter;
    client_counter++;
...
void sendMessage(Message *msg) {
    ...
    sendto(clients[0].socket, msg, sizeof(Message), 0, NULL, 0);
    pthread_mutex_unlock(&client_mutex);
}
...
void *HandleClient(void *arg) {
    ...
    int hello;
    while (recvfrom(client_socket, &hello, sizeof(int), 0, (struct sockaddr *) &address, &len) >= 0 &&
           AllChecked == 0) {

        pthread_mutex_lock(&lib_mutex);
        Client cur = clients[findClientBySocket(client_socket)];
        srand(time(NULL));
        // choosing book
        int randomIndex = (rand() + cur.id) % (M * N * K);
        Book book_to_check = library[randomIndex];
        sendto(client_socket, &book_to_check, sizeof(Book), 0, (struct sockaddr *) &address, &len);
        // received data
        recvfrom(client_socket, &msg, sizeof(msg), 0, (struct sockaddr *) &address, &len);
        ...
        if (library[checked_book.id].checked == 1) {
            ...
            sprintf(msg.text, "Student %d: Book #%d is at line %d, bookcase %d, position %d. It is %s.\n", cur.id,
                    checked_book.id,
